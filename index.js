 ...
 */
const { alfred_preferences, alfred_workflow_uid } = process.env;
	...
	return new Promise((resolve, reject) => {
		fs.readFile( `${jsonDir}/site-statuses.json`, 'utf8', function (err, data) {
			if ( data ) {
				...
				Object.keys(newSiteData).map( key => newSiteData[key]['status'] = data[key] );
				resolve( newSiteData )
			} else {
				reject( Error( err ) )
			}
...
filterSiteData( async data => {
	...
	.map(site => ({
		...
		icon: {
			path: `${alfred_preferences}/workflows/${alfred_workflow_uid}/${site.status === 'running' ? 'icon.png' : 'icon-gray.png'}`
		},
	}));
